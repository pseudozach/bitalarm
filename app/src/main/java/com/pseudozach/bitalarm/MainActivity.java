...
import com.pseudozach.bitalarm.alarms.ui.AlarmsFragment;
import com.pseudozach.bitalarm.alarms.ui.WalletFragment;
import com.pseudozach.bitalarm.data.BaseItemCursor;
import com.pseudozach.bitalarm.list.RecyclerViewFragment;
import com.pseudozach.bitalarm.settings.SettingsActivity;
import com.pseudozach.bitalarm.timepickers.Utils;

import butterknife.BindView;

import static com.pseudozach.bitalarm.list.RecyclerViewFragment.ACTION_SCROLL_TO_STABLE_ID;
import static com.pseudozach.bitalarm.list.RecyclerViewFragment.EXTRA_SCROLL_TO_STABLE_ID;
...
public class MainActivity extends BaseActivity {
    ...
    public static final int    PAGE_WALLET          = 1;
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        setTabIcon(PAGE_WALLET, R.drawable.ic_account_balance_wallet_white_24dp, tabIconColor);
...
// Set up the buttons
    ...
    private void setTabIcon(int index, @DrawableRes int iconRes, @NonNull final ColorStateList color) {
        ...
        if(index == 1) {
            if(mAuth.getCurrentUser() == null){
                ...
                mDatabase.child("user-info").child(mAuth.getCurrentUser().getUid()).child("balance").addValueEventListener(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if(dataSnapshot.exists()){
                            long balance = (long) dataSnapshot.getValue();
                            tab.setText(String.valueOf(balance) + " Sats");
                        } else {
                            tab.setText("0 Sats");
                        }
    ...
    private static class SectionsPagerAdapter extends FragmentPagerAdapter {
        ...
        public Fragment getItem(int position) {
            switch (position) {
                ...
                case PAGE_WALLET:
                    //return new WalletFragment();
                    return new FreshWalletFragment();
                default:
    ...
    protected void onResume() {
        ...
        if(mAuth.getCurrentUser() != null){
            ...
            mDatabase.child("user-info").child(mAuth.getCurrentUser().getUid()).child("balance").addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    Log.e("MA", "get balance single time");
                    if(dataSnapshot.exists()){
                        long balance = (long) dataSnapshot.getValue();
                        Log.e("MA", "balance:: " + balance);
                        tab.setText(String.valueOf(balance) + " Sats");
                    } else {
                        tab.setText("0 Sats");
                    }
